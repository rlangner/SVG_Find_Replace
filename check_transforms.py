...
def find_group_bbox_simple(svg_file, group_id):
    ...
    print(f"Matrix: {group_matrix}")
    
    # Extract scale factor from the transformation matrix
    # For a simple scale + rotation matrix, the scale can be derived from the matrix elements
    a, b, c, d, e, f = group_matrix
    scale_x = math.sqrt(a*a + b*b)
    scale_y = math.sqrt(c*c + d*d)
    print(f"Scale factors: scale_x={scale_x:.6f}, scale_y={scale_y:.6f}")
    ...
    if has_content:
        width = max_x - min_x
        height = max_y - min_y
        print(f"Transformed bbox: width={width:.3f}, height={height:.3f}")
        
        # If we have a uniform scale, we can calculate original dimensions
        if abs(scale_x - scale_y) < 0.0001 and scale_x > 0:
            original_width = width / scale_x
            original_height = height / scale_x
            print(f"Original bbox (before transform): width={original_width:.3f}, height={original_height:.3f}")
...
def main():
    ...
    for group_id in groups:
        ...
        if result:
            ...
            print(f"Final: Group {group_id}: height={height:.3f}, width={width:.3f}")
        else:
