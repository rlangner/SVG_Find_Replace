...
def find_adjacent_groups_at_same_location(first_group, input_groups, input_bboxes, used_input_ids, first_group_id):
    """Find adjacent groups in the SVG that are at the same location as the first group."""
    adjacent_groups = []
    
    # Get the bounding box of the first group
    first_bbox = input_bboxes[first_group_id]
    
    # Calculate center position of the first group
    first_center_x = (first_bbox[0] + first_bbox[2]) / 2
    first_center_y = (first_bbox[1] + first_bbox[3]) / 2
    
    # Tolerance for position matching (in SVG coordinate space)
    position_tolerance = 5.0  # Adjust as needed
    
    # Find groups that have the same or very close center position
    for input_id, input_group in input_groups.items():
        if input_id in used_input_ids or input_id == first_group_id:
            continue  # Skip already used groups and the first group itself
        ...
        distance = ((center_x - first_center_x)**2 + (center_y - first_center_y)**2)**0.5
        
        # If the group is at the same location (within tolerance), add it to adjacent groups
        if distance <= position_tolerance:
            adjacent_groups.append(input_id)
