...
def replace_groups_in_svg(input_svg_path: str, lookup_svg_path: str, output_svg_path: str):
    ...
    for matched_input_groups, find_id in matches:
        ...
        if matched_input_groups:
            ...
            # Calculate the center of the replacement group before any transforms are applied
            original_replacement_center_x, original_replacement_center_y = calculate_group_center([replace_group])

            # If the original replacement group has a transform, we need to account for it
            # to determine where its visual center actually is
            if original_transform:
                # For now, we'll handle simple translate transforms
                # In a more complete implementation, we could parse other transforms too
                import re
                translate_match = re.search(r'translate\s*\(\s*([-\d.]+)\s*,?\s*([-\d.]+)\s*\)', original_transform)
                if translate_match:
                    orig_tx = float(translate_match.group(1))
                    orig_ty = float(translate_match.group(2))
                    # Adjust the center by the original translation
                    original_replacement_center_x += orig_tx
                    original_replacement_center_y += orig_ty
            ...
            translate_x = matched_center_x - original_replacement_center_x
            translate_y = matched_center_y - original_replacement_center_y

            # Create the centering transform
            centering_transform = f"translate({translate_x},{translate_y})"

            # Apply only the centering transform (don't combine with original - we're repositioning)
