#!/usr/bin/env python3
"""
SVG Group Matcher and Replacer - Fixed Positioning
                    ...
                    for j in range(0, len(coords), 6):
                        if j + 5 < len(coords):
                            if command.islower():
                                ...
                                current_x, current_y = coords[j+4], coords[j+5]
                            result.append(f"C {coords[j]:.3f} {coords[j+1]:.3f} {coords[j+2]:.3f} {coords[j+3]:.3f} {current_x:.3f} {current_y:.3f}")
                    ...
                    for j in range(0, len(coords), 4):
                        if j + 3 < len(coords):
                            if command.islower():
                                ...
                                current_x, current_y = coords[j+2], coords[j+3]
                            result.append(f"S {coords[j]:.3f} {coords[j+1]:.3f} {current_x:.3f} {current_y:.3f}")
                    ...
                    for j in range(0, len(coords), 4):
                        if j + 3 < len(coords):
                            if command.islower():
                                ...
                                current_x, current_y = coords[j+2], coords[j+3]
                            result.append(f"Q {coords[j]:.3f} {coords[j+1]:.3f} {current_x:.3f} {current_y:.3f}")
...
def replace_groups_in_svg(input_svg_path: str, lookup_svg_path: str, output_svg_path: str):
    ...
    for matched_input_groups, find_id in matches:
        ...
        if matched_input_groups:
            ...
            # Calculate the transform needed to position the replacement group at the same location
            # as the matched groups in the input SVG
            position_transform = calculate_original_transform(matched_input_groups, input_root)

            # Get the replacement group's original transform (if any) - this is internal to the replacement
            replacement_original_transform = replacement.get('transform', '')

            # For positioning in the new context, we only use the calculated position transform
            # The replacement's original transform is ignored for global positioning
            replacement.set('transform', position_transform)
